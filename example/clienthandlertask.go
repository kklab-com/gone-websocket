...
func (h *ClientHandlerTask) WSConnected(ch channel.Channel, req *http.Request, resp *http.Response, params map[string]any) {
	println(fmt.Sprintf("%s client WSConnected", ch.ID()))
	ch.Write(h.Builder.Ping(nil, nil)).Sync()
}

func (h *ClientHandlerTask) WSDisconnected(ch channel.Channel, req *http.Request, resp *http.Response, params map[string]any) {
